const Experience = () => {
    return (
        <div className="experience">
            <h2>Experience & Education</h2>

            <ul>
                <li>
                    <i>Seton Hall University</i>, 2004-2008
                    <ul>
                        <li>Graduated with 3.00 GPA</li>
                        <li>Bachelor in Arts & Science , Psychology</li>
                        <li>worked in Computer Lab as Field Technician</li>
                    </ul>
                </li>
                <li>
                    <i>Towson University</i>, 2009-2013
                    <ul>
                        <li>Master in Information Systems Management</li>
                        <li>studied network security and cryptography </li>
                    </ul>
                </li>
                <li>
                    <i>Dapp University</i> - <strong>Blockchain Developer Student Intern</strong>, 2020-present
                    <ul>
                        <li>Studied DApp development</li>
                        <li>Assisted Research on Solana &amp; Development on Web3 Products</li>
                        <li>Collaborates with peers using Slack</li>
                        <li>Completed workshops, webinars and weekly lessons on various blockchain topics</li>
                        <li>Completed Capstone DEX, decenralize exchange project</li>
                    </ul>
                </li>
                <li>
                    <i>NFT Social Media Manager</i> - <strong>Davincij15 NFT Davitar</strong>, 2019-2022
                    <ul>
                        <li>Discord Chat creation and monitor</li>
                        <li>write and post Daily Announcements</li>
                        <li>telegram monitor and scam messages removal</li>
                        <li>Weekly report on community engagement and conversation</li>
                    </ul>
                </li>
                 <li>
                    <i>PSB Advanced Technology</i> - <strong>Website Development</strong>, 2010-2016
                    <ul>
                        <li>HTML &amp; HTML5 website development</li>
                        <li>Requirements gathering and communicated with stakeholders on bi-weekly basis</li>
                        <li>researched high volume websites for creative perspective</li>
                        <li>developed website for small businesses</li>
                    </ul>
                </li>
                  <li>
                    <i>PSB Advanced Technology</i> - <strong>Blockchain Advocate & Freelancer</strong>, 2016-2022
                    <ul>
                        <li>solidity backend development research</li>
                        <li>using the Helium blockchain, completed the web3 IoT Project completion; decentralized wireless network </li>
                        <li>researched high volume websites for creative perspective</li>
                        <li>Crypto Mining using ASICs, GPUs and USB sticks</li>
                    </ul>
                </li>
            </ul>
        </div>
    );
}

export default Experience;